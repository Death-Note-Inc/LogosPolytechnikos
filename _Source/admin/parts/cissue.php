...
class Issue {
	...
	public function getActualIssue(){
		$sql = 'SELECT issue.name as issue_name, DATE_FORMAT(issue.date, "%m/%Y") as issue_date, issue.id as issue_id FROM issue WHERE YEAR(issue.date) = YEAR(CURDATE()) OR YEAR(issue.date) = YEAR(CURDATE())-1 ORDER BY issue.id DESC;';
		$query = $this->db->prepare($sql);
		$query->execute();
		if ($query->rowCount() > 0) {
